...
function monitorParams(time, parameters, stage, obsUpdate) {
    return _Rx2.default.Observable.interval(time)
    // .merge(obsUpdate)
    .flatMap(ii => _Rx2.default.Observable.from(parameters).let(getDistinctParameters(stage))).distinctUntilChanged();
}
